...
- ファイル配置: `dashboard/<ダッシュボード名>/dashboard.json` + `sql/` にSQL個別管理
...
with open("dashboard/daily-report/dashboard.json") as f:
    data = json.load(f)
...
```
1. 【必須】リモート同期 → /sync-dashboard でDatabricksから最新取得
...
`databricks lakeview update` を実行する前に、必ず `/sync-dashboard` でリモートの最新状態を取得すること。
...
- [ ] SQLファイルを `dashboard/<name>/sql/` にも保存したか
